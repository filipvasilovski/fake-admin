import { SignInForm } from '@/components/sign-in-form';

// import { UsedPasswordsPreview } from "@/components/used-passwords-preview"

export function SignInPage() {
  return (
    <>
      <div className="lg:container relative h-screen flex-col items-center justify-center grid lg:max-w-none lg:grid-cols-2 lg:px-0">
        <div className="relative hidden h-full flex-col justify-between bg-muted p-10 text-white dark:border-r lg:flex">
          <div className="absolute inset-0 bg-zinc-900" />
          <div className="relative z-20 flex items-center text-lg font-medium">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="mr-2 h-6 w-6"
            >
              <path d="M15 6v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3" />
            </svg>
            Testers Challenge 2024
          </div>

          {/* <UsedPasswordsPreview /> */}

          {/* <div className="relative z-20">
            <blockquote className="space-y-2">
              <p className="text-lg">
                Guessing passwords is like finding a good parking spot in Europe
                — impossible, but we admire your determination. 🚗 Keep trying!
              </p>
            </blockquote>
          </div> */}
        </div>
        <div className="p-8">
          <div className="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px]">
            <div className="flex flex-col space-y-2 text-center">
              <h1 className="text-2xl font-semibold tracking-tight">
                Dashboard Sign In
              </h1>
              <p className="text-sm text-muted-foreground">
                Enter your email & password below to sign in to your account
              </p>
            </div>
            <SignInForm />
          </div>
        </div>
      </div>
    </>
  );
}
